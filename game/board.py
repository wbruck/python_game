...
class Board:
    ...
    def calculate_field_of_view(self, x: int, y: int, vision_range: int) -> Set[Position]:
        """
        Calculate visible positions from a given point within vision range using an optimized
        octant-based field of view algorithm.
        ...
        visible = set()
        center = Position(x, y)
        visible.add(center)  # Always see own position
        
        # Process each octant
        for octant in range(8):
            self._compute_octant(x, y, vision_range, octant, visible)
            
        return visible
        
    def _compute_octant(self, x: int, y: int, vision_range: int, octant: int, visible: Set[Position]):
        """Helper method for field of view calculation in one octant."""
        line_of_sight_cache = {}  # Cache line of sight results within octant
        
        # Pre-calculate octant transformations
        if octant == 0: tx, ty = (1, 0); px, py = (0, 1)    # Top right
        elif octant == 1: tx, ty = (0, 1); px, py = (1, 0)  # Right top
        elif octant == 2: tx, ty = (0, 1); px, py = (-1, 0) # Right bottom
        elif octant == 3: tx, ty = (-1, 0); px, py = (0, 1) # Bottom right
        elif octant == 4: tx, ty = (-1, 0); px, py = (0, -1)# Bottom left
        elif octant == 5: tx, ty = (0, -1); px, py = (-1, 0)# Left bottom
        elif octant == 6: tx, ty = (0, -1); px, py = (1, 0) # Left top
        else: tx, ty = (1, 0); px, py = (0, -1)             # Top left
        
        for distance in range(1, vision_range + 1):
            # Only check positions on the edge of vision range
            cx = x + (distance * tx)
            cy = y + (distance * ty)
            
            for i in range(distance + 1):
                # Calculate position in current octant
                pos_x = cx + (i * px)
                pos_y = cy + (i * py)
                ...
                target = Position(pos_x, pos_y)
                cache_key = (pos_x, pos_y)
                
                # Use cached line of sight result if available
                if cache_key in line_of_sight_cache:
                    if line_of_sight_cache[cache_key]:
                        visible.add(target)
                else:
                    # Calculate and cache line of sight
                    has_los = self._has_line_of_sight(Position(x, y), target)
                    line_of_sight_cache[cache_key] = has_los
                    if has_los:
                        visible.add(target)
